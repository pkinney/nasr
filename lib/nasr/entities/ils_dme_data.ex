defmodule NASR.Entities.ILSDMEData do
  @moduledoc "Entity struct for ILS4 (DISTANCE MEASURING EQUIPMENT DATA) records"
  import NASR.Utils
  ...
  def new(entry) do
    %__MODULE__{
      ...
      operational_status: convert_operational_status(entry.status_of_dme_operational_ifr),
      operational_status_date: parse_date(entry.date_of_dme_operational_status),
      latitude_formatted: entry[:latitude_of_dme_transponder_antenna_formatted] || entry.latitude_of_dme_transponder_antenna,
      latitude_seconds: entry.latitude_of_dme_transponder_antenna,
      longitude_formatted: entry[:longitude_of_dme_transponder_antenna_formatted] || entry.longitude_of_dme_transponder_antenna,
      longitude_seconds: entry.longitude_of_dme_transponder_antenna,
      position_source: convert_source_code(entry.indicating_source_of_latitude_longitude),
      distance_from_approach_end: safe_str_to_int(entry.distance_of_dme_transmitter_antenna),
      distance_from_centerline: safe_str_to_int(entry.of_dme_transponder_antenna),
      ...
      site_elevation: safe_str_to_float(entry.elevation_of_dme_transponder_antenna),
      dme_channel: entry.channel_on_which_distance_data_is_transmitted,
      distance_from_stop_end: safe_str_to_int(entry.distance_of_dme)
    }
  end
