defmodule NASR.Entities.ILS do
  @moduledoc """
  Represents Instrument Landing System (ILS) information from the NASR ILS_BASE data.

  ILS provides precision approach guidance to aircraft by transmitting localizer
  and glideslope signals that define both horizontal and vertical guidance paths
  to the runway. This entity contains basic information about ILS installations
  ...
  * `:runway_length` - Length of Runway associated with ILS (feet)
  * `:runway_width` - Width of Runway associated with ILS (feet)
  * `:category` - ILS Category (I, II, III, etc.)
  * `:owner` - Owner Name of the ILS facility
  * `:operator` - Operator Name of the ILS facility
  * `:approach_bearing` - Magnetic bearing of the ILS approach course (degrees)
  * `:magnetic_variation` - Magnetic Variation at the facility location (degrees)
  * `:magnetic_variation_hemisphere` - Hemisphere for magnetic variation. Values: `:east`, `:west`
  * `:component_status` - Current operational status of the ILS components
  * `:component_status_date` - Date when component status was last updated
  * `:latitude_degrees` - Latitude of ILS facility in degrees
  * `:latitude_minutes` - Latitude of ILS facility in minutes
  * `:latitude_seconds` - Latitude of ILS facility in seconds
  * `:latitude_hemisphere` - Latitude hemisphere. Values: `:north`, `:south`
  * `:latitude_decimal` - Latitude of ILS facility in decimal degrees
  * `:longitude_degrees` - Longitude of ILS facility in degrees
  * `:longitude_minutes` - Longitude of ILS facility in minutes  
  * `:longitude_seconds` - Longitude of ILS facility in seconds
  * `:longitude_hemisphere` - Longitude hemisphere. Values: `:east`, `:west`
  * `:longitude_decimal` - Longitude of ILS facility in decimal degrees
  * `:latitude_longitude_source_code` - Source code for lat/long coordinates
  * `:site_elevation` - Elevation of ILS site above mean sea level (feet)
  * `:localizer_frequency` - ILS Localizer frequency (MHz)
  * `:back_course_status_code` - Status of ILS back course capability
  """
  import NASR.Utils
  ...
  @type t() :: %__MODULE__{
          ...
          owner: String.t(),
          operator: String.t(),
          approach_bearing: float() | nil,
          magnetic_variation: integer() | nil,
          magnetic_variation_hemisphere: :east | :west | String.t() | nil,
