defmodule NASR do
  ...
  @doc """
  A special use case for airports that will return a list of airports with the connected entities populated in the `NASR.Entities.Airport` struct.

  ## Airport-Attachable Entities
  The following entities can be directly attached to airports through various identifiers:
  - Airport submodules (via site_id: site_no + "*" + site_type_code):
    - Airport.ArrestingSystems, Airport.AttendanceSchedule, Airport.Contact, Airport.Remark, Airport.Runway, Airport.RunwayEnd
  - Other entities via site_id (site_no + "*" + site_type_code):
    - AWOS, ClassAirspace, ParachuteJumpingArea
  - ATC facilities (via site_no + site_type_code)
  - ILS systems (via site_no + site_type_code + runway_end_id)
  - Frequencies (via serviced_facility matching arpt_id)
  """
  def list_airports(opts \\ []) do
    {:ok, stream, _file} = open_stream(opts)

    airports =
      [stream: stream, include: ["APT_BASE"]]
      |> stream_entities()
      |> Enum.to_list()

    site_map = Map.new(airports, fn a -> {{a.site_no, a.site_type}, a} end)
    ...
    |> Enum.reduce(site_map, fn
      %NASR.Entities.Airport.Runway{site_no: site_no, site_type: site_type} = runway, acc ->
        case Map.get(acc, {site_no, site_type}) do
          nil ->
            Logger.warning("[#{__MODULE__}] No airport found for runway #{inspect(runway)}
. Skipping...")
            acc

          airport ->
            updated_airport =
              Map.update(airport, :runways, [runway], fn runways ->
                Enum.sort_by([runway | runways], & &1.runway_id)
              end)

            if airport.arpt_id == "DTO" do
              IO.inspect(airport.runways, label: "DTO Runways")
              IO.inspect(updated_airport.runways, label: "DTO Runways")
            end

            Map.put(acc, {site_no, site_type}, updated_airport)
        end

      entity, acc ->
        Logger.warning("[#{__MODULE__}] Unhandled entity #{inspect(entity)}. Skipping...")
        acc
    end)
  end
